...
data = uiimport('5272022-night.TXT'); %replace file name


%- Creating a time vector that is the same length of the data to plot with
time = 5*(1:length(data.data(:,1)));
    % Data was sampled every five minutes, hence the 5

%- Creating vectors to plot exposure limits on
shortTermPM2p5 = 35*ones(length(data.data(:,1)),1);
longTermPM2p5 = 12*ones(length(data.data(:,1)),1);

easLow = 300*ones(length(data.data(:,1)),1);
easAccpt = 500*ones(length(data.data(:,1)),1);
easMarg = 1000*ones(length(data.data(:,1)),1);
easHigh = 3000*ones(length(data.data(:,1)),1);

%- Creating vectors for the bar graph
totalparticles = sum(data.data);
...
figure(1)
plot(time,data.data(:,2),time,shortTermPM2p5,time,longTermPM2p5)
...
figure(4)
hold on
plot(time,4.5*data.data(:,13)) %4.5 converts from ppb to ug/m^3
plot(time,easLow,'g')
