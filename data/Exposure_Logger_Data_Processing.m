...
data = uiimport('5282022-double.TXT'); %replace file name


%- Creating a time vector that is the same length of the data to plot with
time = 5*(1:length(data.x5282022_double(:,1)));
    % Data was sampled every five minutes, hence the 5

%- Creating vectors to plot exposure limits on
shortTermPM2p5 = 35*ones(length(data.x5282022_double(:,1)),1);
longTermPM2p5 = 12*ones(length(data.x5282022_double(:,1)),1);

easLow = 300*ones(length(data.x5282022_double(:,1)),1);
easAccpt = 500*ones(length(data.x5282022_double(:,1)),1);
easMarg = 1000*ones(length(data.x5282022_double(:,1)),1);
easHigh = 3000*ones(length(data.x5282022_double(:,1)),1);

%- Creating vectors for the bar graph
totalparticles = sum(data.x5282022_double)
...
figure(1)
plot(time,data.x5282022_double(:,2),time,shortTermPM2p5,time,longTermPM2p5)
...
figure(2)
plot(time,data.x5282022_double(:,5))
hold on
...
figure(3)
plot(time,data.x5282022_double(:,14))
hold on
xlabel('Time (min)')
...
figure(4)
hold on
plot(time,4.5*data.x5282022_double(:,13)) %4.5 converts from ppb to ug/m^3
plot(time,easLow,'g')
